...
import { EnumSeverity, getErrorMessage, IApproach } from '@packages/common';
...
export const approachApiSlice = createApi({
  ...
  endpoints: (builder) => ({
    getApproachById: builder.query<IApproach, string>({
      query: (id) => `/approach/${id}`,
    }),
    getApproaches: builder.query<IApproach[], void>({
      query: () => '/approach',
      providesTags: [APITags.ApproachList],
    }),
    addApproach: builder.mutation<IApproach, Partial<IApproach>>({
      query: (newApproach) => {
    ...
    updateApproach: builder.mutation<IApproach, Pick<IApproach, 'id'>>({
      ...
      async onQueryStarted(id, { dispatch, queryFulfilled }) {
        const deleteResult = dispatch(
          approachApiSlice.util.updateQueryData(
            ...
            undefined,
            (draft) => {
              return draft.filter((approach: IApproach) => approach.id !== id);
            },
          ),
        );
        try {
