...
import { PhotoRequest } from '../../models/request';
...
router
  ...
  .get(async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await PhotoRequest.findById(id);
...
router
  ...
  .get(async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await PhotoRequest.find();
      res.status(SuccessStatusCode.OK).send(result);
    } catch (error) {
      next(error);
    }
  ...
  .post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const request = new PhotoRequest(payload);
        await request.save();
        res.status(SuccessStatusCode.CREATED).send(request);
      } catch (error) {
        next(error);
      }
    },
  ...
  .put(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const result = await PhotoRequest.findByIdAndUpdate(id, payload, {
          new: true,
          runValidators: true,
        });
        if (!result) {
...
router
  ...
  .delete(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      const result = await PhotoRequest.findByIdAndDelete(id);
      if (!result) {
        return next(HttpError.notFound(ErrorMessages.NoRecordFound));
      }
      res.status(SuccessStatusCode.OK).send(result);
